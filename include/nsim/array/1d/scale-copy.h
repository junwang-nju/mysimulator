...
namespace mysimulator {
  ...
  template <typename T1,typename T2,typename T3>
  void ScaleCopy(Array1D<T1>& V,const Array1D<T2>& V2,const Array1D<T3>& V3,
                 unsigned int n) {
...
namespace mysimulator {
  ...
  template <typename T>
  void BlasScaleCopy(Array1D<T>& V,const Array1D<T>& V2,const Array1D<T>& V3,
                     unsigned int n) {
    BlasCopy(V,V2,n);
    BlasScale(V,V3,n);
  }
  ...
  template <typename T>
  void MemScaleCopy(Array1D<T>& V,const Array1D<T>& V2,const Array1D<T>& V3,
                    unsigned int n) {
    MemCopy(V,V2,n);
    Scale(V,V3,n);
  }
