...
int main() {
  ...
  */
  ...
  Array<InteractionFunc<double,FreeSpace>*> TF;
  TF.Allocate(2);
  for(unsigned int i=0;i<sz[1];++i) {
    for(unsigned int k=0;k<sz[0];++k)
      if((S.Interaction(1).Index(i)[0]==S.Interaction(0).Index(k)[0])&&
         (S.Interaction(1).Index(i)[1]==S.Interaction(0).Index(k)[1])) {
        TF[0]=S.Interaction(0).Function(k);  break;
      }
    for(unsigned int k=0;k<sz[0];++k)
      if((S.Interaction(1).Index(i)[1]==S.Interaction(0).Index(k)[0])&&
         (S.Interaction(1).Index(i)[2]==S.Interaction(0).Index(k)[1])) {
        TF[1]=S.Interaction(0).Function(k);  break;
      }
    S.Interaction(1).Function(i)->SetNeighbor(TF[0],TF[1]);
  }
  Clear(TF);
  TF.Allocate(5);
  for(unsigned int i=0;i<sz[4];++i) {
    for(unsigned int k=0;k<sz[1];++k)
      if((S.Interaction(4).Index(i)[0]==S.Interaction(1).Index(k)[0])&&
         (S.Interaction(4).Index(i)[1]==S.Interaction(1).Index(k)[1])&&
         (S.Interaction(4).Index(i)[2]==S.Interaction(1).Index(k)[2])) {
        TF[0]=S.Interaction(1).Function(k);
        TF[3]=S.Interaction(1).Function(k); break;
      }
    for(unsigned int k=0;k<sz[1];++k)
      if((S.Interaction(4).Index(i)[1]==S.Interaction(1).Index(k)[0])&&
         (S.Interaction(4).Index(i)[2]==S.Interaction(1).Index(k)[1])&&
         (S.Interaction(4).Index(i)[3]==S.Interaction(1).Index(k)[2])) {
        TF[1]=S.Interaction(1).Function(k);
        TF[4]=S.Interaction(1).Function(k);  break;
      }
    for(unsigned int k=0;k<sz[0];++k) 
      if((S.Interaction(4).Index(i)[1]==S.Interaction(0).Index(k)[0])&&
         (S.Interaction(4).Index(i)[2]==S.Interaction(0).Index(k)[1])) {
        TF[2]=S.Interaction(0).Function(k);  break;
      }
    S.Interaction(4).Function(i)->SetNeighbor(TF[0],TF[1],TF[2],TF[3],TF[4]);
  }
  Clear(TF);
  ...
  P->Time(MDTime_TotalPeriod)=1000.;
