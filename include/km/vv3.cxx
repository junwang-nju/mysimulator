...
int main() {
  ...
  Array<NeighborList<double,FreeSpace> > NLA;
  NLA.Allocate(1);
  NLA[0].Allocate(IFN[3],3);
  NLA[0].LoadTarget(S.Interaction(3));
  for(unsigned int i=0;i<sz[3];++i) {
    Value<double>(NLA[0].Parameter(i,CoreLJ612Radius))=4.;
    Value<double>(NLA[0].Parameter(i,CoreLJ612EnergyDepth))=1.;
    NLA[0].ParameterBuild(i);
  }
  for(unsigned int i=0;i<sz[3];++i) {
    NLA[0].Index(i)[0]=S.Interaction(3).Index(i)[0];
    NLA[0].Index(i)[1]=S.Interaction(3).Index(i)[1];
  }
  NLA[0].SetRadius(4,6);
  Pointer<Array<NeighborList<double,FreeSpace> > >(P->Parameter(PropagatorMDWithNL_ListArray))=&NLA;
