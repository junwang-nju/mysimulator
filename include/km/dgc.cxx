...
int main() {
  ...
  do {
    ...
    //for(unsigned int k=0;k<3;++k)   MV[UsedID[i]][k]=BG.Double()*Step;
    SI=(unsigned int)(UG.Double()*NUsed);
    for(unsigned int k=0;k<3;++k) MV[UsedID[SI]][k]=BG.Double()*Step;
    //for(unsigned int i=0;i<NUsed;++i)
    //for(unsigned int k=0;k<3;++k) S.Location()[UsedID[i]][k]+=MV[UsedID[i]][k];
    for(unsigned int k=0;k<3;++k)
      S.Location()[UsedID[SI]][k]+=MV[UsedID[SI]][k];
    GetHessian(S,Hess,N,NB);
    ZPT=GetGT(S,Hess,GT,N,NB,Used,UsedID,NUsed);
    if(ZPT>ZT) {
      //for(unsigned int i=0;i<NUsed;++i)
      //for(unsigned int k=0;k<3;++k)
      //  S.Location()[UsedID[i]][k]-=MV[UsedID[i]][k];
      for(unsigned int k=0;k<3;++k)
        S.Location()[UsedID[SI]][k]-=MV[UsedID[SI]][k];
    } else {
      cout<<"=====  "<<ZPT<<endl;  ZT=ZPT;
    }
    if(ZT<TTH) break;
  } while(true);
