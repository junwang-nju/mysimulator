...
int main() {
  ...
  do{
    GetDV(S,Hess,A,B,IP,N,NB);
  for(unsigned int i=1;i<N-1;++i)
  for(unsigned int k=0;k<3;++k) MV[i][k]=BG.Double();
  for(unsigned int i=1;i<N-1;++i)
  for(unsigned int k=0;k<3;++k) S.Location()[i][k]+=MV[i][k]*0.002;
  ZPT=GetGT(S,Hess,GT,N,NB);
  if(ZPT>ZT) {
    for(unsigned int i=1;i<N-1;++i)
    for(unsigned int k=0;k<3;++k) S.Location()[i][k]-=MV[i][k]*0.002;
    cout<<ZT<<"\t"<<ZPT<<endl;
  } else { ZT=ZPT; cout<<"==============  "<<ZT<<endl; break; }
  } while(true);

  T=GetDV(S,Hess,A,B,IP,N,NB);
  for(unsigned int i=1;i<N-1;++i)
  for(unsigned int k=0;k<3;++k)   S.Location()[i][k]+=1e-4*S.Gradient()[i][k];
  T=GetDV(S,Hess,A,B,IP,N,NB);
  ...
  while(T>-0.60) {
    ...
    TT=0;
    for(unsigned int i=1,n=0;i<N-1;++i)
    for(unsigned int k=0;k<3;++k,++n) TT+=MV[i][k]*B[n];
    for(unsigned int i=1,n=0;i<N-1;++i)
    for(unsigned int k=0;k<3;++k,++n) MV[i][k]-=TT*B[n];
    TT=0;
    for(unsigned int i=1;i<N-1;++i)
    for(unsigned int k=0;k<3;++k) TT+=MV[i][k]*MV[i][k];
    TT=Step/sqrt(TT);
    for(unsigned int i=1;i<N-1;++i)
    for(unsigned int k=0;k<3;++k) MV[i][k]*=TT;
    for(unsigned int i=1;i<N-1;++i)
    for(unsigned int k=0;k<3;++k) S.Location()[i][k]+=MV[i][k];
    PT=GetDV(S,Hess,A,B,IP,N,NB);
    if(PT<T)  { T=PT; cout<<T<<"\t"<<GetGT(S,Hess,GT,N,NB)<<endl; }
    else {
      for(unsigned int i=1;i<N-1;++i)
      for(unsigned int k=0;k<3;++k) S.Location()[i][k]-=MV[i][k];
    }
