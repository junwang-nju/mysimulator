...
namespace std {
  ...
  void EFuncDihedralBase(
    ...
    Distance(X[J],X[I],J,I,B);
    copy(tmvec[0],B.DisplaceVec);
    Distance(X[K],X[J],K,J,B);
    copy(tmvec[1],B.DisplaceVec);
    Distance(X[L],X[K],L,K,B);
  ...
  void GFuncDihedralBase(
    ...
    unsigned int I=idx[0], J=idx[1], K=idx[2], L=idx[3];
    T dsq01,dsq12,dsq23;
    dsq01=Distance(X[J],X[I],J,I,B);
    copy(tmvec[0],B.DisplaceVec);
    dsq12=Distance(X[K],X[J],K,J,B);
    copy(tmvec[1],B.DisplaceVec);
    dsq23=Distance(X[L],X[K],L,K,B);
    copy(tmvec[2],B.DisplaceVec);
    crossProd(tmvec[0],tmvec[1],tmvec[3]);
    crossProd(tmvec[1],tmvec[2],tmvec[4]);
    T nr1,nr2;
