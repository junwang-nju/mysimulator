...
#include "data/interaction/interaction-unit.h"
...
namespace std {
  ...
  void CalcInteraction(
    ...
    for(unsigned int i=0;i<F.size;++i)
      F[i].EFunc(X,idx,F[i].prm.data,F.B,F.Geo,Energy,F[i].tmvec.structure,
                 F.NumMerUnit());
  }
  ...
  void CalcInteraction(
    ...
    for(unsigned int i=0;i<F.size;++i)
      F[i].EFunc(X,idx,F[i].prm.data,F.B,F.Geo,Gradient,F[i].tmvec.structure,
                 F.NumMerUnit());
  }
  ...
  void CalcInteraction(
      ...
      T& Energy, Vector<T>* Gradient) {
    for(unsigned int i=0;i<F.size;++i)
      F[i].EFunc(X,idx,F[i].prm.data,F.B,F.Geo,Energy,Gradient,
