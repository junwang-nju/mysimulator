...
int main() {
  ...
  P.sysPg[0][BV_Temperature].d=0.5;
  P.sysPg[0][BV_RelaxTime].d=0.1;
  P.sysPg[0][BV_DegreeFreedom].d=asum(dMsk);
  for(unsigned int i=0;i<4;++i) P.sysPg[0].merPg[i][MassData].d=2.;
  double fac=0.;
  for(unsigned int i=0;i<4;++i)
    fac+=P.sysPg[0].merPg[i][MassData].d*normSQ(P.V[i]);
  P.sysPg[0][BV_DualKineticEnergy].d=fac;
