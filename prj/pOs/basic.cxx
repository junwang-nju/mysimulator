...
int main() {
  ...
  double e,ke;
  e=0;
  for(unsigned int i=0;i<nunit-1;++i) e+=feneE(X[i],X[i+1],v);
  for(unsigned int i=0;i<nunit;++i)
  for(unsigned int j=i+2;j<nunit;++j) e+=repE(X[i],X[j],v);
  for(unsigned int i=0;i<nunit;++i)   e+=sphereE(X[i],v);
  ke=0.5*normSQ(V);
